...
def load_base(db:Database,username:str)->Tuple[int,int,int,int,int,int,int,int,int,str,int]:
    data:tuple=(username,)
    template:str="""select c.hp,c.atk,c.speed,c.mana,p.add_hp,p.add_atk,p.add_speed,p.add_mana,p.coins,c.nazev,p.current_hp from 
...
def save_stats(db:Database,player:Player)->None:
    data:Tuple[str]=(player.current_hp,player.add_hp,player.add_atk,player.add_speed,player.add_mana,player.coins,player.username)
    template:str="""UPDATE player SET current_hp=%s, add_hp=%s, add_atk=%s, add_speed=%s, add_mana=%s,coins=%s WHERE username=%s;"""
    with db.mydb.cursor() as cursor:
        cursor.execute(template,data)
        db.mydb.commit()